...
def _sales_pred(item) -> bool:
    return authlib.has_role(item, "sales") or "営業" in (item.get("department") or "")
...
_EKITAN_BASE = "https://ekitan.com"  # 確認用URLの組み立て
...
def _push_messages(user_id: str, messages: list) -> None:
    """任意の message オブジェクト列を push（テキスト＋DLボタン等）."""
    _line_post(_LINE_PUSH_URL, {"to": user_id, "messages": messages[:5]})
...
def _compute(site: str):
    """現場への待機要員の通勤比較を実行。戻り値 (rows or None)。"""
    repo = DynamoDBStaffRepository()
    staffs = repo.list(StaffStatus.AVAILABLE)
    if not staffs:
        return None
    return compare_site(site, staffs, _source(), _REGISTRY, strategy=Strategy.CHEAPEST)


def _ekitan_url(from_id, to_id) -> str:
    return "%s/transit/fare/sf-%s/st-%s" % (_EKITAN_BASE, from_id, to_id)


def _format_reply(site: str, rows: list) -> str:
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f"　経由: {res.route_summary}"  # 路線名つき。〔徒歩〕は徒歩連絡
        )
        if r.from_id and r.to_id:           # ② 自分で確認できる ekitan のURL
            lines.append(f"　🔎確認: {_ekitan_url(r.from_id, r.to_id)}")
    if has_walk:                            # ③ 乗換回数と経由駅数の差を説明
        lines += ["", "※ 経由の〔徒歩〕は駅間の徒歩連絡です。乗換回数には含みません"
                      "（電車の乗換だけを数えるため、経由駅数より少なくなります）。"]
    return "\n".join(lines)


def _export_commute_csv(site: str, rows: list) -> str:
    """比較結果を CSV にして S3 へ。key を返す（DLリンク用）。"""
    key = "exports/commute_%s.csv" % _time.strftime("%Y%m%d_%H%M%S")
    _s3.put_object(Bucket=_bucket(), Key=key,
                   Body=to_csv(site, rows).encode("utf-8-sig"),
                   ContentType="text/csv; charset=utf-8")
    return key


def _worker(text: str, user_id: str, base: str = "") -> None:
    """非同期側: 重い比較を実行し push で後送りする（結果＋DLボタン）."""
    try:
        rows = _compute(text)
        if not rows:
            _push(user_id, "待機中(available)の要員がいません。")
            return
        messages = [{"type": "text", "text": _format_reply(text, rows)[:4900]}]
        # ① 結果CSVのダウンロードボタン（base があるとき）
        if base:
            try:
                key = _export_commute_csv(text, rows)
                messages.append({
                    "type": "template", "altText": "通勤コスト比較(CSV)",
                    "template": {"type": "buttons",
                                 "text": "この比較結果(CSV)をダウンロードして確認できます",
                                 "actions": [{"type": "uri", "label": "結果CSVダウンロード",
                                              "uri": _dl_link(base, key)}]}})
            except Exception as e:  # noqa: BLE001
                print("[worker] csv export error:", repr(e))
        _push_messages(user_id, messages)
    except TransitDataError as e:
        _push(user_id,
              f"「{text}」を現場として認識できませんでした。\n"
              f"駅名をそのまま送ってください（例: 東京 / 新宿。中国語・かな・ローマ字も可）。\n（{e}）")
    except Exception as e:  # noqa: BLE001
        _push(user_id, f"エラーが発生しました: {type(e).__name__}: {e}")
...
def _handle_dl(event: dict) -> dict:
    ...
    if key and sig and hmac.compare_digest(_sign_key(key), sig):
        fname = key.rsplit("/", 1)[-1] or "staff.csv"   # commute_*.csv / staff_*.csv
        url = _s3.generate_presigned_url(
            "get_object",
            Params={"Bucket": _bucket(), "Key": key,
                    "ResponseContentDisposition": 'attachment; filename="%s"' % fname,
...
def handler(event: dict, context) -> dict:
    ...
    if event.get("_worker"):
        _worker(event.get("text", ""), event.get("userId", ""), event.get("base", ""))
        ...
        if user_id:
            ...
            try:
                _lambda.invoke(
                    ...
                    Payload=json.dumps({"_worker": 1, "text": site, "userId": user_id,
...
def _worker_sync_fallback(reply_token: str, text: str) -> None:
    """非同期不可時の同期返信（遅くなり得るが最後の手段。DLボタンは付かない）."""
    try:
        rows = _compute(text)
        msg = _format_reply(text, rows) if rows else "待機中(available)の要員がいません。"
    except Exception as e:  # noqa: BLE001
