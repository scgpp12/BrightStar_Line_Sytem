...
# 催促 push 専用の token 参数（設定時は別channel=社員のtokenで送る。未設定は自channel）
PUSH_TOKEN_PARAM = os.environ.get("PUSH_TOKEN_PARAM", "")
...
def push_token():
    """催促などのpush送信に使うtoken。
    PUSH_TOKEN_PARAM が設定されていれば別channel(社員)のtokenを使い、
    リマインドが社員アシスタント側に届くようにする。未設定なら自channel。"""
    return _get_param(PUSH_TOKEN_PARAM) if PUSH_TOKEN_PARAM else line_token()
